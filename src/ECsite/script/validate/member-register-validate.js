...
const creditCardNumberError = document.getElementById(
...
function toggleCreditCardFields(isCreditSelected) {
    const creditCardFields = document.querySelectorAll('.credit_debit-input');
    const securityCodeField = document.getElementById('security-code');

    for (const field of creditCardFields) {
        field.required = isCreditSelected;
    }

    // セキュリティコードの必須属性も切り替えた後にフォーム全体のバリデーションを実行
    securityCodeField.required = isCreditSelected;
    validateForm();
}
